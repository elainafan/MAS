"""Train IPPO on Quadrapong.

Usage:
    python scripts/train_ippo.py [--config src/configs/ippo.yaml]
"""
import sys
import os
import argparse
...
def main(config: dict):
    ...
    logger = Logger(
        log_dir=cfg_l["log_dir"],
        algo_name="ippo",
        use_wandb=cfg_l.get("use_wandb", False),
        wandb_project=cfg_l.get("wandb_project", "quadrapong"),
        config=config,
    )
    logger.print(f"IPPO training on {device}")
    ...
    use_cnn = env.use_cnn
    obs_shape = env.obs_shape if use_cnn else None  # (C, H, W) for CNN
    logger.print(f"Obs dim={obs_dim}, use_cnn={use_cnn}, Actions={act_dim}, Agents={num_agents}")
    ...
    trainer = IPPOTrainer(
        obs_dim=obs_dim, action_dim=act_dim, num_agents=num_agents,
    ...
    trainer.obs_shape = obs_shape  # for reshape in training updates
    ...
    while global_step < total_steps:
        ...
        for step in range(rollout_steps):
            ...
            values = trainer.get_values(obs_batch_flat)
            ...
            buffer.insert(obs=obs_batch_flat, actions=actions, action_log_probs=log_probs,
                          rewards=reward_arr, values=values, dones=done_arr,
        ...
        last_value = trainer.get_values(last_obs_batch)

        data = buffer.get_training_data()
        advantages, returns = buffer.compute_gae(last_value, last_done_arr)
        ...
        if global_step % cfg_e["eval_interval"] < rollout_steps:
            ...
            eval_m = evaluate(eval_env, {"shared": trainer.actor},
                              ...
                              video_path=str(Path(cfg_l["log_dir"]) / f"ippo/videos/step_{global_step}.mp4"))
            eval_env.close()
            ...
            if winrate > best_eval:
                best_eval = winrate
                trainer.save(str(ckpt_dir / "ippo_best.pt"))

        # Checkpoint
        if global_step % cfg_l["save_interval"] < rollout_steps:
            trainer.save(str(ckpt_dir / f"ippo_step{global_step}.pt"))
...
def _train_ppo_update(trainer, data, advantages, returns):
    """Standard PPO update with clipped objective. Handles both RAM and CNN obs."""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="src/configs/ippo.yaml")
